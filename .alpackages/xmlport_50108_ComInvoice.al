xmlport 50108 CommInvoiceImport
{
    ...
    schema
    {
        textelement(root)
        {
            tableelement(PurHdr; "Sales Header")
            {
                ...
                textelement(FInalAmount) { MinOccurs = Zero; }
                textelement(Due) { MinOccurs = Zero; }
                ...
                begin
                    ...
                    if (FInalAmount.Trim() = '') or (FInalAmount.Trim() = '0') then
                        FInalAmt := 0
                    else
                        Evaluate(FInalAmt, FInalAmount);
                    ...
                    tblSalesHdr.setrange("Sell-to Customer No.", tblCustomer."No.");
                    tblsaleshdr.SetRange("External Document No.", ContractNos);
                    tblsaleshdr.SetRange("Package Tracking No.", doctype);
                    ...
                    if booNotFirstLine = false then begin
                        ...
                        tblsaleshdr.Validate("Package Tracking No.", doctype);
                        ...
                        // Sales Line
                        if Doctype = 'Provisional Commercial Invoice' then begin
                            ...
                            tblSalesLine.Validate(Quantity, DryWrights);
                            tblSalesLine.Validate("Unit Price", Price95p);
                            tblSalesLine.Modify(true);
                        end else
                            if Doctype = 'Final Commercial Invoice' then begin
                                ...
                                tblSalesLine.Insert(true);
                                tblSalesLine.type := tblSalesLine.type::" ";
                                tblSalesLine.Description := '100 PCT OF FINAL COMMERCIAL INVOICE VALUE: ' + Format(FinalCommInvValue100P);
                                tblSalesLine.Modify(true);
                                ...
                                tblSalesLine.Insert(true);
                                tblSalesLine.type := tblSalesLine.type::" ";
                                tblSalesLine.Description := '95 PCT OF THE SHIPPED GOODS VALUE: ' + Format(ShippedGoodValue95P);
                                tblSalesLine.Modify(true);

                                tblSalesLine.Init();
                                tblSalesLine."Document Type" := tblSalesLine."Document Type"::Invoice;
                                tblSalesLine."Document No." := DocNo;
                                tblSalesLine."Line No." := 40000;
                                tblSalesLine.Insert(true);
                                tblSalesLine.type := tblSalesLine.type::"G/L Account";
                                tblSalesLine.Validate("No.", '6110');
                                tblSalesLine.Validate(Description, 'FINAL AMOUNT DUE');
                                tblSalesLine.Validate(Quantity, DryWrights);
                                tblSalesLine.Validate("Unit Price", FInalAmt / DryWrights);
                                tblSalesLine.Modify(true);
